...
def deleter():
    ...
    try :
        Sensor.objects.filter(pub_date__lte=(now-datetime.timedelta(weeks=1))).delete()
    except :
    ...
    try :
        Hours_sensor.objects.filter(pub_date__lte=(now-datetime.timedelta(weeks=4))).delete()
    except :
        print(f"Erro : deleting cron job : sensor. \n")
   try :
        Days_sensor.objects.filter(pub_date__lte=(now-datetime.timedelta(weeks=8))).delete()
    except :
        print(f"Erro : deleting cron job : sensor. \n")
    try :
        Weeks_sensor.objects.filter(pub_date__lte=(now-datetime.timedelta(weeks=24))).delete()
    except :
        print(f"Erro : deleting cron job : sensor. \n")
